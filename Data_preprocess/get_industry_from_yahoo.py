...
def need_get(df):
    for index, row in df.iterrows():
        ...
        df.at[index, 'Mark'] = get_industry(stock_name)
...
def get_industry(ticker):
    ...
    while attempts < 3:
        try:
            ...
            industry = info.loc[[0], ['sector', 'industry']]
            industry['Empresa'] = ticker
            # industry.to_csv("Data/" + ticker + "_info.csv")
            industry.to_csv("Data/"+"industry_info.csv", index=False, mode='a' if os.path.exists("Data/"+"industry_info.csv") else 'w', header=not os.path.exists("Data/"+"industry_info.csv") )
